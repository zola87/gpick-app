...
// Helper to initialize GoogleGenAI strictly from process.env.GEMINI_API_KEY
...
export const analyzeGachaImage = async (base64Image: string, apiKey?: string): Promise<{ name: string; priceJPY: number; box_2d?: [number, number, number, number] }[]> => {
  try {
    ...
    const prompt = `
      這是一張日本扭蛋機台的照片。照片中可能包含一台或多台不同的扭蛋機。
      請幫我辨識出照片中所有的扭蛋機台，並回傳一個 JSON 陣列。
      
      針對每一台辨識出的扭蛋機，請提供：
      1. name: 根據機台上的圖片或文字，給這個扭蛋一個簡短、好懂的中文商品名稱，且名稱中必須包含「扭蛋」二字（例如：「吉伊卡哇 扭蛋」、「寶可夢 睡眠扭蛋」、「三麗鷗 捏捏樂扭蛋」）。不要超過 15 個字。
      2. priceJPY: 辨識機台上的日幣價格數字（通常是 300, 400, 500 等）。如果找不到，請預設為 300。
      3. box_2d: (選填) 該扭蛋機台在圖片中的 2D 邊界框，格式為 [ymin, xmin, ymax, xmax]，數值範圍為 0-1000。請盡可能精確框選出該機台的展示區域。
    `;
          ...
          items: {
            ...
            properties: {
              ...
              box_2d: {
                ...
                description: "2D 邊界框 [ymin, xmin, ymax, xmax]"
              }
            },
            required: ["name", "priceJPY"]
          }
        }
      }
    });
    ...
    try {
      ...
      console.error("Failed to parse Gacha JSON:", text);
      return [];
    }
  } catch (error) {
    console.error("Error analyzing gacha image:", error);
    throw error;
  }
};

export const reanalyzeHistoricalReport = async (
  report: SalesReport,
  apiKey?: string
): Promise<string> => {
  try {
    ...
    const prompt = `
      我是一個日本代購賣家。這是一份歷史連線的銷售報表，請根據以下數據給我一段簡短的商業回顧與鼓勵：
      
      報表名稱：${report.name}
      總營收：$${report.totalRevenue}
      淨利：$${report.totalProfit}
      總件數：${report.totalItems}
      
      請提供一段專業、鼓勵的繁體中文分析，點評這次的毛利率（淨利/總營收），並給予未來連線的建議。
    `;
