...
import { X, Edit2, AlertCircle, Package, Loader2, Check, Lock, ChevronLeft, ChevronRight, Search, Gift, ExternalLink } from 'lucide-react';
        ...
        <main className="flex-1 overflow-y-auto pb-24 md:pb-6">
          ...
          {activeTab === 'profile' && (
            <div className="max-w-lg lg:max-w-xl mx-auto px-4 md:px-8 py-5 md:py-8 space-y-3">
              ...
              {/* Community links */}
              {(settings.communityLine || settings.communityInstagram) && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {settings.communityLine && (
                      <a
                        href={settings.communityLine}
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: '#fff', border: '1.5px solid #B3CF99', borderRadius: 16, textDecoration: 'none', cursor: 'pointer' }}
                      >
                        <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#06C755', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <LineIcon size={20} color="#fff" />
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#2c2c34' }}>LINE 匿名社群</div>
                          <div style={{ fontSize: 11, color: '#a89c94', marginTop: 1 }}>第一手商品資訊都在社群裡，歡迎加入</div>
                        </div>
                        <ExternalLink size={14} style={{ color: '#c2b6aa', flexShrink: 0 }} />
                      </a>
                    )}
                    {settings.communityInstagram && (
                      <a
                        href={settings.communityInstagram}
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: '#fff', border: '1.5px solid #D4B2F6', borderRadius: 16, textDecoration: 'none', cursor: 'pointer' }}
                      >
                        <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg, #f59e0b, #ec4899, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#2c2c34' }}>Instagram</div>
                          <div style={{ fontSize: 11, color: '#a89c94', marginTop: 1 }}>商品照不定期更新，歡迎追蹤</div>
                        </div>
                        <ExternalLink size={14} style={{ color: '#c2b6aa', flexShrink: 0 }} />
                      </a>
                    )}
