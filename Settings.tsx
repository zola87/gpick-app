...
import { Save, Settings as SettingsIcon, Plus, X, Archive, AlertCircle, Download, ChevronDown, ChevronRight, MessageSquare, Tag, ListFilter, Layers, Cloud, Database, RefreshCw, Send, Loader2, Camera, Users } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 pb-20">
      ...
      <div className="space-y-4">
        ...
        <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider px-1 pt-2">🌐 品牌社群</p>
        <CollapsibleSection title="社群連結設定" icon={Users}>
          <p className="text-xs text-stone-400 mb-4 leading-relaxed">
            設定後會在客人「我的」頁面底部顯示加入社群的入口，讓客人更方便找到你的社群。
          </p>
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-stone-400 mb-1">LINE 社群／群組邀請連結</label>
              <input
                type="url"
                value={localSettings.communityLine || ''}
                onChange={(e) => handleChange('communityLine', e.target.value || undefined)}
                placeholder="https://line.me/ti/g2/..."
                className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm"
              />
              <p className="text-xs text-stone-400 mt-1">LINE 社群邀請連結，或群組加入連結。留空則不顯示。</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-stone-400 mb-1">Instagram 連結</label>
              <input
                type="url"
                value={localSettings.communityInstagram || ''}
                onChange={(e) => handleChange('communityInstagram', e.target.value || undefined)}
                placeholder="https://www.instagram.com/yourhandle"
                className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm"
              />
              <p className="text-xs text-stone-400 mt-1">你的 Instagram 帳號頁面連結。留空則不顯示。</p>
            </div>
          </div>
        </CollapsibleSection>
